...
import com.gleb.vinnikov.social_network.repos.PostRepo;
...
public class PostingService {
    ...
    private void validate(PostingRequest request) {
        if (request.getHeader() == null && request.getImage() == null && request.getPostText() == null) {
            throwIllegalArgumentException("posting.no.fields.provided");
        }
    ...
    private void validateHeader(String header) {
        if (header.length() > headerMaxLength) {
            throwIllegalArgumentException("posting.header.is.too.long");
        }
    }
